...
/**
 * End-to-end integration tests that run an embedded Tomcat server and establish an actual WebSocket session using
 * {@link org.springframework.web.socket.client.standard.StandardWebSocketClient}. as well as a simple STOMP/WebSocket client created to support these tests.
 *
 * The test strategy here is to test from the perspective of a client connecting to a server and therefore it is a much more complete test. However, writing and maintaining these
 * tests is a bit more involved.
 *
 * An all-encapsulating strategy might be to write the majority of tests using server-side testing (either standalone or with Spring configuration) with end-to-end integration
 * tests serving as a higher-level verification but overall fewer in number.
 *
